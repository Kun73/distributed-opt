...
```{r,m-w, fig.cap="Master-Workers Architecture"}
...
we first bound $(\Vert\bar x_{k+1}-x^*\Vert_2, \Vert X_{k+1}-\boldsymbol1\bar x_{k+1}\Vert_R,\Vert Y_{k+1}-v\bar y_{k+1}\Vert_C)^T$ by a linear system of inequalities in terms of their previous values. Then based on lemma \@ref(lem:lem37), we derive how should we choose the step sizes $\alpha_i,i=1,2,...,n$ so that $\rho(A)<1$.
...
In this chapter, we define the matrix norm of $X\in\mathbb R^{n\times p }$ using definition \@ref(der:normpp).
