...
Under assumption \@ref(exr:muL) , there exists an unique solution to \@ref(eq:obj) $x^*\in\mathbb{R}^{1\times p}$. To prove the convergence of those methods, the idea is to bound three quantities, namely $\bar x_{k+1}-x^*$, $X_{k+1}-\mathbf{1}\bar x_{k+1}$, and $Y_{k+1}-\mathbf{1}\bar y_{k+1}$ by the linear combination of their previous values under corresponding measurements. This will introduce a matrix $A$.  In order to make it converge, we need to make $\rho(A)<1$, i.e. the spectral radius of $A$ to be less than $1$(similar idea with contraction mapping), which will derive constraints to the stepsize $\alpha$. By doing so, the authors prove the convergence of those three methods and derive their convergence rate. 
