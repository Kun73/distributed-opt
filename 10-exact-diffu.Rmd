...
Next we do a non-asymptotic analysis to check the convergence of $(\bar z_k,\check z_k)^T$ under a decreasing stepsize $\alpha_k:=\frac{1}{k+m}$.
...
\begin{align}
E \left[\Vert \bar z_{k+1}\Vert^2\right]\leq (1-\frac{3}{2}\alpha_k\mu)\Vert \bar z_k\Vert^2 + \frac{3\alpha_k c^2\Vert \mathcal{K}_{R,u}\Vert^2\delta^2}{n\mu}\Vert \check z_k\Vert^2 + \frac{\alpha_k^2\sigma^2}{n}
\end{align}
...
&\quad + \frac{\alpha_k^2\mathcal{D}_1\mathcal{K}_L\mathcal{B}_l}{c^2}E\left[\Vert \mathcal{S}_k(\mathcal{X}_k)\Vert^2\right]
(\#eq:checkz)
\end{align}
...
&\quad + \left[(1+\gamma)\lambda'+(1+\frac{1}{\gamma})({2c\alpha_k^2\lambda'\Vert \mathcal{K}_L\Vert^2\delta^2})\right]\Vert \check z_k\Vert^2
(\#eq:checkz2)
\end{align}