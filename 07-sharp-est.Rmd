...
(\#eq:introsldsgd)
\end{equation}
where $\tilde k$ is some shift of $k$. The goal is to show that asymptotically, $\frac{1}{n}\sum\limits_{i=1}^nE\left[\Vert x_i(k)-x^*\Vert^2\right]=R'(k)=U(k)+\frac{1}{n}V(k)$ has the same convergence rate with $R(k)$ in SGD. Notice $V(k)$ can be shown to decay faster than $U(k)$, so if we can bound $\hat W(1-\rho_w^2)$ by another quantity $C$ which does not depende on $\rho_w^2$, i.e. does not depende on the network, then we can have 
\begin{equation}
