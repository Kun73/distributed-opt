...
This is because given such conditions on $\Vert X(0)-\mathbf{1}x^*\Vert^2$ and $\Vert \nabla F(\mathbf{1}x^*)\Vert^2$, $\hat X=\mathcal{O}(n)$ and $\hat W=\mathcal{O}(\frac{1}{1-\rho_w^2})$, then $\hat V=\mathcal{O}(\frac{n\rho_w^2}{(1-\rho_w^2)^3})$.

Recall $R'(k)=U(k)+\frac{1}{n}V(k)$, from theorem \@ref(thm:asymUk), lemma \@ref(lem:sublrdsgd), and lemma \@ref(lemma:hatVdsgd), we show 
\begin{equation}
...
```{remark}
$\mathcal{O}\left(\frac{1}{\left(1-\rho_{w}^2\right)^{2}}\right)$ or $\mathcal{O}\left(\frac{1}{\left(1-\rho_{w}\right)^{2}}\right)$?
```
