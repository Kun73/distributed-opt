...
Take $c=\frac{3}{8}\alpha_k\mu$ and let $\alpha_k\leq\min\{\frac{1}{L},\frac{1}{3\mu}\}$ in \@ref(eq:Uk1), we derive lemma \@ref(lem:Uk1). $\alpha_k\leq \frac{1}{L}$ is to make $\lambda = 1-\alpha_k\mu$ in lemma \@ref(lem:contraction2L), $\alpha_k\leq \frac{1}{3\mu}$ is to make $(1+c)\lambda^2\leq 1-\frac{3}{2}\alpha_k\mu$. Thus we derive lemma \@ref(Uk1).
...
First we show $\exists c=\mathcal{O}(\frac{1}{n}),s.t. E\left[\Vert x(k)-x^*\Vert^2\right]\leq \frac{c_3}{k},\forall k\geq K_2:=\left\lceil\frac{\theta L}{\mu}\right\rceil$. Then by induction, lemma \@ref(lem:prodineq) and bound for the sums, we have the convergence rate for algorithm \@ref(exm:sgd).

```{theorem, sublrsgd}
Under algorithm \@ref(ex:sgd), suppose $k\geq K_2$, we have 
$$
\mathbb{E}\left[\left\|x(k)-x^{*}\right\|^{2}\right] \leq \frac{\theta^{2} \sigma^{2}}{(2 \theta-1) n \mu^{2} k}+\mathcal{O}\left(\frac{1}{n}\right) \frac{1}{k^{2}}
$$
```

Compare theorem \@ref(thm:sublrsgd) and theorem \@ref(thm:impRp)(or formula \@ref(eq:nimpRp)), we derive the transient time $K_T=\mathcal{O}(\frac{n}{(1-\rho_w^2)^2})$ for algorithm \@ref(exm:dsgd) with $\alpha_k=\frac{\theta}{\mu(k+K)}$ to reach the aymptotic rate of convergence of algorithm \@ref(exm:sgd).
