...
The corallary \@ref(cor:transienttimedsgd) indicates that for DSGD with $\alpha_k=\frac{\theta}{\mu(k+K)},\theta>2$, when $k\geq K_T$, $p(\mathcal{G})=\mathcal{O}(1)$, this is what we call **asymptotic network independence**. Hence for a problem where a usual centralized implementation requires a large number of iteration, we can consider a distributed one with asymptotic network independence property. As a result, both methods demand the same computational power but the distributed one enjoys more desirable properties.
