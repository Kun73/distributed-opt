...
&\leq\|\nabla F(\mathbf{x}(k))-\mathbf{1} h(X(k))\|^{2}+n \sigma^{2}\\

\|\nabla F(\mathbf{x}(k))-\mathbf{1} h(X(k))\|^{2}&\leq \Vert \nabla F(X(k))\Vert^2
(\#eq:xbar1dsgdl1)
\end{align}
and
\begin{align}
\Vert \nabla F(X(k))\Vert^2&\leq
\left(L\|\mathbf{x}(k)-\mathbf{1} \bar{x}(k)\|+\sqrt{n} L\left\|\bar{x}(k)-x^{*}\right\|+\left\|\nabla F\left(\mathbf{1} x^{*}\right)\right\|\right)^{2}\\
&\leq 2 L^{2}\|\mathbf{x}(k)-\mathbf{1} \bar{x}(k)\|^{2}+4 n L^{2}\left\|\bar{x}(k)-x^{*}\right\|^{2}+4\left\|\nabla F\left(\mathbf{1} x^{*}\right)\right\|^{2}
(#eq:xbar1dsgdl2)
\end{align}

Use the two inequalities \@ref(eq:xbar1dsgdl1) and \@ref(eq:xbar1dsgdl2) in \@ref(eq:xbardsgd), and let $c=\frac{1-\rho_w^2}{2}$(the choice of $c$ in \@ref(eq:ineq2dsgt) in chapter \@ref(sec:dsgt)), we have 
