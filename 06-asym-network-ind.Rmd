...
Let $Q(\cdot):\mathbb{R}^{p}\to\mathbb{R}^p$ be a specific compression operator, which is known and satisfy assumption \@ref(exr:contractionEQ).
...
The CHOCO-SGD method can be seen in algorithm \@ref(exm:chocoG).
```{example, chocoG, name = "CHOCO-SGD"}
